...
class HRnet(nn.Module):
    def __init__(self,num_sources,ch_per_soruce=3):
        ...
        self.conv83to26_l2=nn.Conv2d(in_channels=self.out_ch_line_1,out_channels=self.out_ch_line_4,stride=3,padding=8,kernel_size=24)
    ...
    def inception_module14(self,input):
        part1=self.activ(self.conv113_l14(input))
    ...
    def inception_module10(self,input):
        ...
        out=torch.cat((part1,part2,part3,part4),axis=1)
        out=self.BN_l1(self.activ(self.conv101Last_l10(out)))
        # out=self.BN(torch.cat((part1,part2,part3,part4),axis=2))
        return out
    #----------------------------LINE 2 BELOW--------------------------
    ...
    def inception_module38(self,input):
        part1=self.activ(self.conv113_l38(input))
    ...
    def forward(self,input):
        ...
        line3cat=torch.cat([line3,self.tconv26to83(line4)],dim=1)
        line3=self.inception_module38(line3cat)
        line2cat=torch.cat([line2,self.tconv83to126(line3)],dim=1)
        line2=self.inception_module28(line2cat)
        line1cat=torch.cat([line1,self.tconv83to126(line2)],dim=1)
        line1=self.inception_module19(line1)
        out=self.inception_module10(line1)
